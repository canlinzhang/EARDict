# -*- coding: utf-8 -*-
"""
Created on Fri Oct  9 23:45:14 2020
...
#store all the basic structures (r,t)
...
#tntanglement dictionary stores the entangled two_hop EAS 
#for each one_hop EAS.
#We won't work with the entanglement between two_hop EASs
#entanglement are also sets in dictionary
#etgl = {}
#for id_ in basic_set:
#    etgl[id_] = set({})

#build the two_hop set from one_hop
for i in range(num_e):
    for index_0 in one_hop[i]:
        value_0 = inverse_structure_dict[index_0]
        for index_1 in one_hop[value_0[1]]:
            value_1 = inverse_structure_dict[index_1]
            combo = (value_0[0],value_1[0],value_1[1])
            if combo not in structure_dict:
                structure_dict.update({combo:index})
                inverse_structure_dict.update({index:combo})
                index += 1
            index_ = structure_dict[combo]
            #etgl[index_0].add(index_)
            #etgl[index_1].add(index_)
            if (np.abs(value_0[0]-value_1[0]) != num_r) or (
                    value_1[1] != i): #not trivil for entity i
                if index_ not in two_hop[i]:
                    two_hop[i].add(index_)
#    if i <= 1000 and i % 10 == 0:
#        print('fill in two-hop structure', i)
    if i % 100 == 0:
        print('fill in two-hop structure', i)
...
print('total two hop', index - len(basic_set))
print('non trivil two hop', len(self_dict)-len(basic_set))
...
for i in range(num_e):
    for id_0 in one_hop[i]:
        value = inverse_structure_dict[id_0]
        ...
        for id_1 in two_hop[i]:
            if id_1 not in regular_dict[id_0]:
                combo = inverse_structure_dict[id_1]
                #not entangled
                if combo[0] != value[0]:
                    temp = self_dict[id_0].intersection(self_dict[id_1])
                    regular_dict[id_0].update({id_1:len(temp)})
...
for key in test:
    ...
    r_id = key[1]
    t_id = key[2]
    prob = new_prob(s_id,r_id,t_id)
    holder.append([(s_id,r_id,t_id),prob])
    for j in range(num_e):
        if ((s_id,r_id,j) not in train) and ((s_id,r_id,j) not in valid) and ((s_id,r_id,j) not in test):
            prob = new_prob(s_id,r_id,j)
            holder.append([(s_id,r_id,j),prob])
    holder_ = Sort(holder,-1)
    if holder_[0][0] == (s_id,r_id,t_id):
        Hit_1_0 += 1.
    for j in range(3):
        if holder_[j][0] == (s_id,r_id,t_id):
            Hit_3_0 += 1.
            break
    for j in range(10):
        if holder_[j][0] == (s_id,r_id,t_id):
            ...
            break
    j_ = 0
    while holder_[j_][0] != (s_id,r_id,t_id):
        j_ += 1
    MRR_score_0 += 1./float(j_+1)
    del(holder,holder_)
    i += 1
    if i % 10 == 0:
        print('reasoning forward', i)

#predict the source entity
i = 0
for key in test:
    holder = []
    s_id = key[0]
    ...
    holder.append([(s_id,r_id,t_id),prob])
    for j in range(num_e):
        if ((j,r_id,t_id) not in train) and ((j,r_id,t_id) not in valid) and ((j,r_id,t_id) not in test):
            prob = new_prob(j,r_id,t_id)
            holder.append([(j,r_id,t_id),prob])
    holder_ = Sort(holder,-1)
    if holder_[0][0] == (s_id,r_id,t_id):
        Hit_1_1 += 1.
    for j in range(3):
    ...
    while holder_[j_][0] != (s_id,r_id,t_id):
        j_ += 1
    MRR_score_1 += 1./float(j_+1)
    del(holder,holder_)
    i += 1
    if i % 10 == 0:
        print('reasoning backward', i)

MRR_score_0 = MRR_score_0/float(len(test_))
MRR_score_1 = MRR_score_1/float(len(test_))
MRR_score = (MRR_score_0 + MRR_score_1)/2.

Hit_1_0 = Hit_1_0/float(len(test_))
Hit_1_1 = Hit_1_1/float(len(test_))
Hit_1 = (Hit_1_0 + Hit_1_1)/2.

Hit_3_0 = Hit_3_0/float(len(test_))
Hit_3_1 = Hit_3_1/float(len(test_))
Hit_3 = (Hit_3_0 + Hit_3_1)/2.

Hit_10_0 = Hit_10_0/float(len(test_))
Hit_10_1 = Hit_10_1/float(len(test_))
Hit_10 = (Hit_10_0 + Hit_10_1)/2.

print('total reasoning:', len(prob_dict))
print('cpd reasoning', count_2)
print('single rule', len(sgl_rule_dict))
print('compound rule', len(cpd_rule_dict))
print('promoting', count_prom)
print('repelling', count_repe)

print(' ')

print('pred t', 'pred s', 'ave')
print('MRR',MRR_score_0,MRR_score_1,MRR_score)
print('Hit_1',Hit_1_0,Hit_1_1,Hit_1)
print('Hit_3',Hit_3_0,Hit_3_1,Hit_3)
print('Hit_10',Hit_10_0,Hit_10_1,Hit_10)
...
'''
            ...
            if m-k > 0:
                ...
                for ele in descend_dict[id_1]:
                    if id_2_count >= count_bd:
                        break
                    elif (ele[0] != id_0) and (ele[0] not in equal_dict[id_0]) and (
                          ele[0] in basic_set):
                        id_2 = ele[0]
                        #b and c
                        ...
                        if len(str_) != 0:
                            for s in temp_1a2:
                                update(s,id_0,prob_a,temp_set_a,str_)
                            for s in temp_1n2:
                                update(s,id_0,prob_n,temp_set_n,str_)
                        id_2_count += 1
'''
...
'''
                    ...
                    k_n, m_n = k-k_a, m-m_a
                    prob_n = float(k_n)/float(m_n)
                    repe_bd_n, prom_bd_n = regular_bd(m_n,n,num_e)
                    temp_set_n = {id_1,-1*id_2}
                    str_n = 'indpt'
                    if k_n > prom_bd_n:
                        str_n = 'prom'
                    ...
                    if str_n != 'indpt':
                        for s in temp_1n2:
                            update(s,id_0,prob_n,temp_set_n,str_n)
'''
